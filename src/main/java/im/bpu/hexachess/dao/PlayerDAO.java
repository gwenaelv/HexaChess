...
public class PlayerDAO extends DAO<Player> {
	...
	public Player create(Player obj) {
		String requete = "INSERT INTO players (player_id, handle, email, password_hash, rating, "
	...
	public Player update(Player obj) {
		String requete = "UPDATE players SET handle = ?, email = ?, password_hash = ?, rating = ?, "
			+ "is_verified = ? WHERE player_id = ?";
		try {
			PreparedStatement pstmt = connect.prepareStatement(requete);
	...
	public Player read(String id) {
		...
		try {
			...
			if (rs.next()) {
				p = new Player(rs.getString("player_id"), rs.getString("handle"),
	...
	public ArrayList<Player> readAll() {
		...
		try {
			...
			while (rs.next()) {
				list.add(new Player(rs.getString("player_id"), rs.getString("handle"),
					rs.getString("email"), rs.getString("password_hash"), rs.getInt("rating"),
					rs.getBoolean("is_verified"),
					rs.getTimestamp("joined_at") != null
						? rs.getTimestamp("joined_at").toLocalDateTime()
